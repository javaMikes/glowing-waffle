...
var MPlayer = (function () {
	function MPlayer(settings,callback) {
		...
		function bindEvents () {
			...
			/*解决on绑定事件阻止冒泡事件异常*/
			$(document).on('click', ".mp-list tr", function () {
				var index = $(this).index();
				$this.play(parseInt($this.audio.attr('data-displayList')),$(this).index());
				$.ajax({
					url : "addOnePlaytimeOfSong",
					data : {
						songId : player.list[0][index].id
					}
				});
			});
	...
	$.extend(MPlayer.prototype,{
		...
		_setInfo: function (list,song) {
			...
			console.log(songInfo.src);
			// 设置当前播放歌曲
			$this.audio.attr({
//				'src':songInfo.src,
//				'src':'findLocalMusic?musicPath=' + encodeURI(encodeURI(songInfo.src)),
//				'src':'/song/杨洋 - 微微一笑很倾城.mp3',
				'src':songInfo.src,
...
//			$this.dom.cover.attr('src',songInfo.img);
			$this.dom.cover.attr('src','findLocalImg?imgPath=' + encodeURI(encodeURI(songInfo.img)));
			// 更换歌词
